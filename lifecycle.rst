...
omit the details, but it should be clear from the example what each
stage does. (Recall that ``Kind`` is Kubernetes in Docker, while the
``Check for PENDING transactions`` stage waits for asynchronous output
from a backend subsystem.) Groovy's ``post`` construct instructs
Jenkins what to do upon completion of the pipeline.

.. literalinclude:: code/roc-api-tests.groovy

One thing to notice is that this is another example of a tool's usage
of terminology not aligning perfectly with our conceptual usage. Each
conceptual *stage* in :numref:`Figure %s <fig-pipeline>` is
implemented by a Groovy-defined *pipeline*, that in turn consists of a
sequence of Groovy-defined *stages*, which as we can see in this
example, are quite low-level.

This particular pipeline is invoked by a time-based trigger, as
specified in the job template defined by the following YAML file:

.. literalinclude:: code/trigger-time.yaml

To complete the picture, the follow code snippet from another YAML
file shows how a repo-based trigger is specified.

.. literalinclude:: code/trigger-event.yaml

This one happens to executes a different pipeline, which we are not
showing here. Note both example YAML files also specify parameters to
their respective pipelines (not shown).
